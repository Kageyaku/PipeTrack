...
const loadReports = async () => {
  ...
  try {
    const response = await fetch("http://192.168.254.115/liwad-api/reports/get_user_reports.php", {
      method: "POST",
      headers: { 
    ...
    if (!result.canceled) {
      ...
      try {
        const response = await fetch(
          "http://192.168.254.115/liwad-api/upload_avatar.php",
          {
            method: "POST",
...
const handleDeleteIssue = (report_id: string | number) => {
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            const response = await fetch("http://192.168.254.115/liwad-api/reports/archive_report.php", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.254.115/liwad-api/change_password.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: user.user_id,
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch("http://192.168.254.115/liwad-api/update_profile.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  user_id: user.user_id,
            ...
            <TouchableOpacity onPress={() => setShowImageModal(true)}>
              {avatarUri ? (
                ...
                <Image
                  style={styles.avatar}
                  source={{
                    uri: `http://192.168.254.115/liwad-api/${profile.profile}`,
                  }}
                />
              ) : (
          ...
          {avatarUri || profile.profile ? (
            <Image
              source={{
                uri:
                  avatarUri ||
                  `http://192.168.254.115/liwad-api/${profile.profile}`,
              }}
...
{showMap && selectedIssue.location_lat && selectedIssue.location_lng && (
  <View style={{ height: 300, width: "100%", borderRadius: 12, marginTop: 10, overflow: 'hidden' }}>
    <WebView
      source={{ uri: 'http://192.168.254.115/leaflet_map.html' }}
      injectedJavaScript={`
        ...
        window.postMessage(JSON.stringify({
          ...
          image: "${selectedIssue.image_path 
            ? `http://192.168.254.115/liwad-api/uploads/${selectedIssue.image_path.replace(/^uploads\//, '')}` 
            : ''}",
          popupOptions: {
            maxWidth: 220,
            minWidth: 180,
